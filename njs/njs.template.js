...
(function(){
	...
	function _newT(tmplName , context , fs , func , etc )
	{
		...
		this.source     = fs; 
		this.func		= func;
		this.toString   = function() { return "$njs Template [" + tmplName + "]"; }
	}
	...
	var nextStmt = function(body,index,left_delimiter,right_delimiter,special_delimiter)
	{
		...
		if(c_end != -1)
			return {start : start , end : c_end + right_delimiter.length , prefix : prefix , suffix : body.substring(c_end , c_end + right_delimiter.length) ,content : body.substring(c_start + 1,c_end)};
		else
			return {start : -1 , end : -1 , prefix : null ,suffix : null, content : null}
	}
	var _parseText = function(body, tmplName, etc)
	{
		...
		while(true)
		{
			...
			else if((lastStmt.content.charAt(0) == "/" && lastStmt.content.substr(1,11).search(etc.statementTag) == 0) || lastStmt.content.substr(0,10).search(etc.statementTag) == 0)
			{
			...
			else if(lastStmt.prefix.charAt(0) == "$")
			{
				emitSectionText(body.substring(lastStmtIndex , lastStmt.start), funcText);
